...
namespace QSDMS.Util.Excel
{
    ...
    public class ExcelHelper
    {
        ...
        /// <returns></returns>
        public static DataTable ExcelImport(Stream stream, int firstdataindex = 1)
        {
            DataTable dt = new DataTable();
            ...
            IRow headerRow = sheet.GetRow(firstdataindex - 1);
            int cellCount = headerRow.LastCellNum;
            ...
            for (int i = (sheet.FirstRowNum + firstdataindex); i <= sheet.LastRowNum; i++)
            {
                IRow row = sheet.GetRow(i);
                DataRow dataRow = dt.NewRow();
